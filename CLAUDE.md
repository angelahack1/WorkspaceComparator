...
- **Phase 2 — Same filename, different directory.** For each remaining left file, look at right files with the *same filename*. Run the **deterministic** comparison. If `confidence=='high'` and `similarity > 85` → accept as `deterministic`. Otherwise **ask the LLM**; if it scores `>= 70` → accept as `llm_verified`. If the LLM is unreachable (returns `-1`) but deterministic `> 40`, fall back to accepting as `deterministic`. Best candidate wins.
- **Phase 3 — Similar filename (fuzzy), same extension.** For still-unmatched files, compare filenames with `compute_filename_similarity` (SequenceMatcher on the base name). Require `>= 0.70` similarity and matching extension. Score = `filename_sim*30 + content_sim*0.70` (a blended heuristic). High-confidence deterministic matches auto-accept; medium/low with filename sim `> 0.80` escalate to the LLM.
...
Only invoked for genuinely ambiguous Phase 2/3 cases (to keep latency down). `compare_with_llm(name_a, content_a, name_b, content_b) -> int` sends a carefully engineered prompt to Ollama's `/api/generate` (model `glm-5.2:cloud`, `temperature=0.05`, `num_predict=16`, 120 s timeout) asking for **a single integer 0–100** (correspondence %). The prompt explicitly tells the model to *ignore* migration-expected differences (package/namespace moves, import changes, `javax`↔`jakarta`, logging-framework swaps, semantic-preserving renames) and to judge functional purpose, core logic, API surface, naming signals, and architectural role.
...
3. **Performance.** Every candidate comparison re-reads both files from disk (`_read_file` is called repeatedly, uncached), and large workspaces can trigger many LLM calls (120 s timeout each). A big migration comparison can take minutes — the frontend loading overlay waits up to 300 s in tests. File reads and LLM results are good caching targets.
